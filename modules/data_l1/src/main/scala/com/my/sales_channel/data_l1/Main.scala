package com.my.sales_channel.data_l1
...
object Main
  ...
  private def makeBaseDataApplicationL1Service(
    ...
    BaseDataApplicationL1Service(new DataApplicationL1Service[IO, SalesUpdate, SalesChannelStateOnChain, SalesChannelCalculatedState] {
      override def validateUpdate(update: SalesUpdate)(implicit context: L1NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = LifecycleSharedFunctions.validateUpdate[IO](update)

      override def validateData(state: DataState[SalesChannelStateOnChain, SalesChannelCalculatedState], updates: NonEmptyList[Signed[SalesUpdate]])(implicit context: L1NodeContext[IO]): IO[DataApplicationValidationErrorOr[Unit]] = ().validNec.pure[IO]

      override def combine(state: DataState[SalesChannelStateOnChain, SalesChannelCalculatedState], updates: List[Signed[SalesUpdate]])(implicit context: L1NodeContext[IO]): IO[DataState[SalesChannelStateOnChain, SalesChannelCalculatedState]] = state.pure[IO]
      ...
      override def getCalculatedState(implicit context: L1NodeContext[IO]): IO[(SnapshotOrdinal, SalesChannelCalculatedState)] = calculatedStateService.getCalculatedState.map(calculatedState => (calculatedState.ordinal, calculatedState.state))

      override def setCalculatedState(ordinal: SnapshotOrdinal, state: SalesChannelCalculatedState)(implicit context: L1NodeContext[IO]): IO[Boolean] = calculatedStateService.setCalculatedState(ordinal, state)

      override def hashCalculatedState(state: SalesChannelCalculatedState)(implicit context: L1NodeContext[IO]): IO[Hash] = calculatedStateService.hashCalculatedState(state)
